...
(function() {
  ...
  if (code || session) {
    ...
    socket.on('nextDay', handleNextDay);
  ...
  function selectGuild(id) {
    ...
    let nextDayList = document.getElementById('nextDayList');
    if (!nextDayList) {
      nextDayList = document.createElement('div');
      nextDayList.id = 'nextDayList';
      if (!unfoldedElements.includes(nextDayList.id)) {
        nextDayList.classList.add('folded');
      }
      nextDayList.classList.add('guildSection');
      nextDayList.style.zIndex = 1;
      guildBody.insertBefore(nextDayList, dayList.nextSibling);
    }
    ...
    if (!checkPerm(guild, null, null)) {
      ...
      if (guild.members.length < 300) {
        for (let i in guild.members) {
          ...
          /* } else if (i == 300) {
            let overflow = document.createElement('h3');
            overflow.innerHTML = 'Number of players limited to 300';
            leftPlayerList.appendChild(overflow); */
          }
        }
      } else if (
      ...
      // Search
      // let searchEventSubSection = document.getElementById('searchEvents');
      // if (!searchEventSubSection) {
      //   searchEventSubSection = document.createElement('div');
      //   searchEventSubSection.id = 'searchEvents';
      //   if (!unfoldedElements.includes(searchEventSubSection.id)) {
      //     searchEventSubSection.classList.add('folded');
      //   }
      //   searchEventSubSection.classList.add('guildSection');
      //   searchEventSubSection.classList.add('guildSubSection');
      //   eventList.insertBefore(searchEventSubSection,
      //   eventTitle.nextSibling);
      // }

      // let searchEventsTitle = document.getElementById('searchEventsTitle');
      // if (!searchEventsTitle) {
      //   searchEventsTitle = document.createElement('h3');
      //   searchEventsTitle.id = 'searchEventsTitle';
      //   searchEventsTitle.innerHTML = 'Search for Event';
      //   searchEventsTitle.classList.add('title');
      //   searchEventsTitle.onclick = foldHandler;
      //   searchEventSubSection.insertBefore(
      //       searchEventsTitle, searchEventSubSection.children[0]);
      // }

      // let searchEventsContainer =
      //     document.getElementById('searchEventsContainer');
      // if (!searchEventsContainer) {
      //   searchEventsContainer = document.createElement('div');
      //   searchEventsContainer.classList.add('section');
      //   searchEventsContainer.id = 'searchEventsContainer';
      //   searchEventSubSection.appendChild(searchEventsContainer);
      // }
      // makeSearchEventContainer(searchEventsContainer, guild.hg);
      ...
      if (!defaultEventSubSection) {
        ...
        //     defaultEventSubSection, searchEventSubSection.nextSibling);
        eventList.insertBefore(
    ...
    let nextDayTitle = document.getElementById('nextDayTitle');
    if (!nextDayTitle) {
      nextDayTitle = document.createElement('h2');
      nextDayTitle.id = 'nextDayTitle';
      nextDayTitle.innerHTML = 'Next Day';
      nextDayTitle.classList.add('title');
      nextDayTitle.onclick = foldHandler;
      nextDayList.insertBefore(nextDayTitle, nextDayList.children[0]);
      const nextDayContainer = document.createElement('div');
      nextDayContainer.classList.add('section');
      nextDayContainer.id = 'nextDayContainer';
      nextDayList.insertBefore(nextDayContainer, nextDayTitle.nextSibling);
    }
    dragging.update(selectedGuild);
    updateDaySection(guild, true);
    updateNextDaySection(guild);
    setTimeout(function() {
  ...
  /**
   * Make the view that allows searching for events.
   * @private
   * @param {HTMLElement} container The container to replace.
   * @param {SpikeyBot~HungryGames~GuildGame} game The game data to make
   * searchable.
   */
  // function makeSearchEventContainer(container, game) {
  //   container.innerHTML = '';
  //   if (!Fuse) {
  //     let noSearch = document.createElement('a');
  //     noSearch.innerHTML = 'Oops! The search engine failed to load!';
  //     container.appendChild(noSearch);
  //     return;
  //   }
  //   if (!game) return;
  //   // Replace search engine on every load because the search domain may have
  //   // changed.
  //   const searchable =
  //       makeEventsSearchable(defaultEvents, 'default')
  //           .concat(makeEventsSearchable(game.customEventStore, 'custom'));
  //   fuse = new Fuse(searchable, eventSearchOpts);

  //   let searchBar = document.createElement('input');
  //   searchBar.id = 'eventSearchBar';
  //   searchBar.type = 'text';
  //   searchBar.oninput = onEventSearchChange;
  //   searchBar.onkeyup = function(event) {
  //     if (event.keyCode == 27) this.blur();
  //   };
  //   searchBar.placeholder = 'Type to search...';
  ...
  // }
  /**
   * Re-format the event data to make it more helpful to search through.
   * @private
   * @param {Object} events A set of events to re-format. (All custom events or
   * all default events are expected at once).
   * @param {string} topCat The event category. (Default or custom)
   * @return {Object[]} The events reformatted for searching.
   */
  // function makeEventsSearchable(events, topCat) {
  //   let categories = Object.keys(events);
  //   let searchable = [];
  //   for (let i = 0; i < categories.length; i++) {
  //     if (categories[i] === 'battle') continue;
  //     let defaults = Object.values(eventStore[categories[i]] || {});
  //     if (['weapon', 'arena'].includes(categories[i])) {
  //       if (categories[i] === 'weapon') {
  //         defaults = defaults.map((w) => {
  //           return w.outcomes.map((o) => {
  //             o.parentId = w.id;
  //             return o;
  //           });
  //         });
  //       } else if (categories[i] === 'arena') {
  //         defaults = defaults.map((d) => d.outcomes.map((o) => {
  //           o.parentId = d.id;
  //           return o;
  //         }));
  //       }
  //       defaults = [].concat(...defaults);
  //     }
  //     let newVals = defaults.map(function(el) {
  //       el.cat = categories[i];
  //       el.topCat = topCat;
  //       return el;
  //     });
  //     searchable = searchable.concat(newVals);
  //   }
  //   return searchable;
  // }
  /**
   ...
   * @private
   */
  // function onEventSearchChange() {
  //   let container = document.getElementById('eventSearchResults');
  //   let result = fuse.search(this.value);
  //   container.innerHTML = '';
  //   for (let i = 0; i < result.length && i < 10; i++) {
  //     if (!result[i].message && result[i].cat === 'weapon') {
  //       const par = getEvent(result[i].parentId);
  //       result[i].message =
  //           weaponMessage.replace(/\{action\}/g, result[i].action)
  //               .replace(/\{weapon\}/g, par.name);
  //     }
  //     let newRow = makeEventRow(i, result[i], false, result[i].type);
  //     newRow.children[0].onclick = (function(event) {
  //       return function() {
  //         goToEvent(event);
  //         return false;
  //       };
  ...
  /**
   * Make a specific event visible in the list of all events.
   * @private
   * @param {SpikeyBot~HungryGames~Event} event The event to find and show.
   */
  // function goToEvent(event) {
  //   let eventList = document.getElementById('eventList');
  //   eventList.classList.add('folded');
  //   foldHandler.call(eventList.firstChild);
  //   let topFold = document.getElementById(event.topCat + 'Events');
  //   topFold.classList.add('folded');
  //   foldHandler.call(topFold.firstChild);
  //   let minorFold = document.getElementById(event.cat + event.topCat);
  //   minorFold.classList.add('folded');
  //   foldHandler.call(minorFold.firstChild);

  //   let eventPool = defaultEvents[event.cat];
  //   if (event.topCat === 'custom') {
  //     let guild = guilds[selectedGuild];
  //     eventPool = guild.hg.customEventStore[event.cat];
  //   }
  //   let pageNum;
  //   if (['weapon', 'arena'].includes(event.cat)) {
  //     pageNum = eventPool.findIndex((el) => el === event.parentId);
  //   } else {
  //     pageNum = Math.floor(eventPool.findIndex((el) => el === event.id) /
  //     10);
  //   }

  //   selectEventPage(
  //       pageNum, minorFold.children[1], eventPool, event.topCat, event.cat);
  // }
  /**
  ...
  /**
   * Handle the next day of a game being updated.
   * @private
   * @param {string|number} gId The gulid ID of the day update.
   * @param {Object} day The day information.
   * @param {Object} incUsers The current users included in the game.
   */
  function handleNextDay(gId, day) {
    if (selectedGuild != gId) return;
    if (!day || day.num < 0) return;
    const guild = guilds[gId];
    if (!guild.hg) {
      guild.hg = {
        currentGame: {nextDay: day},
      };
    } else {
      guild.hg.currentGame.nextDay = day;
    }
    updateNextDaySection(guild);
  }
  /**
  ...
  function handleDayState(gId, num, state, eventState) {
    if (selectedGuild != gId) return;
    console.log('Day state update', gId, num, state, eventState);
    const guild = guilds[gId];
  ...
  /**
   * Update the next day preview of the game.
   * @private
   * @param {Object} guild The guild of which to update the game data.
   */
  function updateNextDaySection(guild) {
    let container = document.getElementById('nextDayContainer');
    if (!container) return;
    if (!guild.hg || !guild.hg.currentGame.nextDay.events ||
        guild.hg.currentGame.nextDay.events.length == 0) {
      container.innerHTML = '';

      let buttonsRow = document.createElement('div');
      buttonsRow.style.textAlign = 'right';
      buttonsRow.textContent = 'Buttons go here';
      container.appendChild(buttonsRow);

      container.appendChild(
          document.createTextNode('No next day data is available'));
    } else {
      container.innerHTML = '';
      ...
      let list = guild.hg.currentGame.nextDay.events;
      for (let i in list) {
        if (!list[i]) continue;
        container.appendChild(makeDayEventRow(list[i], i, 0));
      }
    }
  }
  /**
